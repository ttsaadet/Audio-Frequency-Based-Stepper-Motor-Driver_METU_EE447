...
OutStr
	PUSH	{R0-R4}						; store current register conditions
...
loop
	LDRB	R2, [R5],#1					; load character, post inc address
...
waitD
	...
	; Restore PLL
	LDR		R1,=SYSCTL_RCC2_R
	POP		{R0}						; restore PLL state
	STR		R0,[R1]
	
	

	POP		{R0-R4}						; restore processor state
	BX		LR							; return
